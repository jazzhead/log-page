...
script
	...
	on _show_report(current_process, test_groups_passed, did_fail_test) --> void
		if did_fail_test then
			...
			if not __SKIP_INFO__ then
				set m to "Test number " & test_num & " failed (test " & group_count & " of test group " & my name & "/" & test_group & "). The rest of the tests were not run." & return & return & �
					"Tests passed:" & return & return �
					& my _format_group_list(test_groups_passed)
				set alert_type to critical
			end if
		else
			if not __SKIP_INFO__ then
				set m to "All " & _test_count & " tests passed successfully." & return & return & �
					"Tests passed (by group):" & return & return �
					& my _format_group_list(test_groups_passed)
